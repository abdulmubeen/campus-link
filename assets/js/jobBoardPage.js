...
window.addEventListener("DOMContentLoaded", (event) => {
  ...
  function attachFilterTagCloseClick() {
    document
      .querySelectorAll("#filter-tags-list .close-span")
      .forEach((close, index) => {
  ...
  function tagClicked(clickedTag) {
    ...
    let exists = Array.from(filterList.querySelectorAll("li")).some((li) =>
      li.textContent.includes(tagText)
    );
    if (!exists) {
  ...
  function refreshList() {
    let filterTags = Array.from(
      document.querySelectorAll("#filter-tags-list li")
    ).map((li) => li.textContent.replace("×", "").trim());
    let jobCards = document.querySelectorAll("#job-list .job-card");

    jobCards.forEach((card) => {
      let cardTags = Array.from(
        card.querySelectorAll(".job-card__tags li")
      ).map((li) => li.textContent);
      let isMatch = filterTags.every((filterTag) =>
